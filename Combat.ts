...
export class Combat {
  ...
  public mobFight(creator: Creator, enemy: Enemy, inventory: Inventory): void {
    ...
    while (enemy.stats.health > 0 || creator.stats.health > 0) {
      ...
      console.log(`⎳ ⎲  `);
      console.log(
        `⎳ ⎲        ${creator.nick}                             |                         Monstro:  ${enemy.name}  `,
      );
      console.log(
        `⎳ ⎲       Vida: ${creator.stats.health}/${creator.stats.max_health}                          |                         Vida: ${enemy.stats.health}  `,
      );
      console.log(
        `⎳ ⎲          Força: ${creator.stats.strength}                       |                         Força: ${enemy.stats.strength}  `,
      );
      console.log(
        `⎳ ⎲          Armadura: ${creator.stats.armor}                     |                         Armadura: ${enemy.stats.armor}  `,
      );
      ...
      if (enemy.stats.health <= 0) {
        ...
        console.log(`XP atual: ${creator.stats.xp}/${creator.stats.max_xp}`);
        break;
      }
    }
  }
  ...
  public bossFight(creator: Creator, enemy: Enemy): void {
    ...
    while (enemy.stats.health > 0 || creator.stats.health > 0) {
      ...
      if (choice == "3") {
        ...
        console.log("");
        console.log(
          `- ${enemy.name} : "Você é um fracote mesmo, HAHHAHAHAHAH, EU VOU TE PEGAR!!!!`,
        );
